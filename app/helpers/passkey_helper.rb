...
module PasskeyHelper
  ...
  private
    ...
    def passkey_error_messages(error: {}, cancellation: {}, duplicate: {})
      ...
      messages = tag.p(error_message, hidden: true, **error_attributes) +
        tag.p(cancellation_message, hidden: true, **cancellation_attributes)

      if duplicate[:message]
        duplicate_message, duplicate_attributes = build_passkey_error_options("duplicate", duplicate)
        messages += tag.p(duplicate_message, hidden: true, **duplicate_attributes)
      end
    ...
    def build_passkey_error_options(type, options)
      ...
      attributes[:class] = [ attributes[:class], "passkey-error" ].compact.join(" ")
      attributes[:data] ||= {}
