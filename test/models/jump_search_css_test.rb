...
class JumpSearchCssTest < ActiveSupport::TestCase
  ...
  test "reader chrome is a bottom bar that can tuck" do
    ...
    assert_match(/\.reader:has\(\.reader-chrome\.is-tucked\)\s*\{[^}]*--reader-bottom-pad:\s*calc\(1\.35rem \+ env\(safe-area-inset-bottom, 0px\)\)/, css)
    desktop = css[/@media \(min-width: 641px\) \{([\s\S]*?)\n\}/]
    assert desktop
    assert_match(/\.reader \{ --reader-bottom-pad: 8rem; \}/, desktop)
    assert_match(/\.reader\.is-chrome-tucked,[\s\S]*--reader-bottom-pad:\s*calc\(1\.35rem \+ env\(safe-area-inset-bottom, 0px\)\)/, desktop)
