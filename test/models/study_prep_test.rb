...
class StudyPrepTest < ActiveSupport::TestCase
  test "groups a chapter into three or four BSB chunks without Kruger or interrogation" do
    ...
    assert_includes payload[:markdown], "Hosted text: BSB."
    ...
    refute_match(/\b(warm-?up|google map|houston|achilles|interrogat)\b/i, payload[:markdown])
  ...
  test "group sheet asks from the text with bold Qs and italic private Paths" do
    ...
    refute_match(/\b(warm-?up|google map|houston|achilles|interrogat)\b/i, payload[:markdown])
    refute_match(/\b(warm-?up|google map|houston|achilles|interrogat)\b/i, payload[:brief])
    ...
    assert_equal "text", noted[:paths].first[:kind]
    assert_equal "note", noted[:paths].last[:kind]
    assert note_path[:text].length <= 90
    assert_match(/\*\*.+\*\*/, payload[:markdown])
    assert_includes payload[:markdown], "_**Paths:** (private)"
    assert_includes payload[:markdown], "your note —"
    payload[:sections].each do |section|
      ...
      if section[:observations].empty?
        ...
        section[:questions].each do |question|
          ...
          assert_includes payload[:markdown], "**#{question[:text]}**"
        end
      end
    end
  end
  ...
  test "hebrews 12 pack is the default leader sheet with bold Qs and private Paths" do
    library = Library.create!
    ...
    assert_includes markdown, "_**Paths:** (private)"
    assert_includes markdown, "your note —"
    refute_includes markdown, "PRIVATE_LANDING_MUST_NOT_DUMP"
    refute_match(/\b(warm-?up|google map|houston|achilles|interrogat)\b/i, markdown)
    ...
    noted.each do |section|
      ...
      section[:questions].each do |question|
        ...
        assert_includes markdown, "**#{question[:text]}**"
      end
    end
    empty.each { |section| assert_empty section[:questions] }
  end

  test "personal study stays a 1:1 press and skips group facilitation" do
    library = Library.create!
    create_note!(library, "jhn.1.14", "The Word became flesh. Jesus is FROM God.")
    ...
    refute_includes payload[:markdown], "_**Paths:**"
    refute_match(/leave a gap/i, payload[:brief])
    assert_match(/one plain question/i, payload[:brief])
